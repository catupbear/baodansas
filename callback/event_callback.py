...
def create_event_callback_blueprint(path: str):
    ...
    def verify_url():
        ...
        logger.info("[event] 收到URL验证请求: timestamp=%s, nonce=%s", timestamp, nonce)
        ret, reply = _crypto.verify_url(msg_signature, timestamp, nonce, echostr)
        if ret != 0:
            logger.error("[event] URL验证失败, 错误码: %d", ret)
            return "验证失败", 403
        logger.info("[event] URL验证成功")
        return reply
    ...
    def receive_event():
        ...
        if ret != 0:
            logger.error("[event] 回调消息解密失败, 错误码: %d", ret)
            return "解密失败", 403
        ...
        try:
            ...
            logger.info("[event] 事件: Event=%s, ChangeType=%s, ChatId=%s",
            ...
            logger.error("[event] 解析回调XML失败: %s", e)
            return "解析失败", 500
...
def _handle_group_change(chat_id: str):
    ...
    try:
        ...
        if contacts is None:
            logger.warning("[event] 无可用通讯录，跳过群 %s", chat_id)
            return
        ...
        if not name or name in ("__skipped__", "__unresolvable__"):
            logger.info("[event] 群 %s 未取到有效群名(%r)，跳过", chat_id, name)
            return
        ...
        if name_changed:
            logger.info("[event] 群名变化: %s -> 「%s」", chat_id, name)
            auto_monitor_match.check_and_sync_group(_db, chat_id, name, _corpid)
    except Exception:
        logger.exception("[event] 处理群变更失败: %s", chat_id)
