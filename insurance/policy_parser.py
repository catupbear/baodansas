...
def _is_valid_person(val: str) -> bool:
    ...
    if re.search(r'公司|集团|合伙|工厂|商行|服务行|修理行|维修行|经营行|贸易行|百货行|批发行|批发部|五金行|建材行|机电行|文具行|商贸|车行(?!驶)|车队|事务所|经营部|经营店|经销商|专营店|服务部|加工厂|养殖场|合作社|回收站|收购站|废品站|加油站|加气站|充电站|换电站|维修站|服务站|研究院|研究所|设计院|分院|医院|卫生院|学院|小学|中学|大学|幼儿园|学校|村民委员会|居民委员会|村委会|居委会|管理处|中心|个体工商户|协会|商会|学会|联合会|促进会|基金会', val) or val_no_paren.endswith(('店', '厂', '铺')) or has_location_paren:
        # 公司名允许长一些，但不能超过 30 字
...
def _identify_policy_type(text: str) -> Optional[str]:
    ...
    # 显式"保险产品名称"标签（众安在线等格式：标题是通用的"电子保险单"，不含具体险种名，
    # 真正的产品名在正文"保险信息"区域用"保险产品名称 XXX"标出，靠标签直接取值最可靠）
    m = re.search(r"保险产品名称\s*([^\n]{2,30}?)\s*(?:\n|保险期间|$)", text)
    if m:
        return m.group(1).strip()
...
def _extract_insured_pingan(text: str, text_merged: str, fields: dict):
    ...
    # 平安交强险格式：被保险人与值以空格分隔、无冒号（"被保险人 深圳市宝安区温师傅电器修理行"）。
    # "被保险人身份证号码…"行因"被保险人"后紧跟"身份"无空格，不会命中 \s+，故安全排除。
    # 逐行遍历（而非 re.search 全文），避免命中失败后错失后续有效行。
    for line in lines:
        s = line.strip()
        m = re.match(r'^被保险人\s+(\S+)', s)
        if m:
            val = _clean_person_name(m.group(1))
            if _is_valid_person(val):
                fields["被保险人"] = val
