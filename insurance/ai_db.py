...
# 注意：模板末尾附带参考 JSON 示例，示例值格式与 policy_parser 解析结果对齐（保险期间用「 至 」连接、
# 核定载客带「人」、核定载质量带「千克」等），保证 AI 输出与系统字段可直接比对
DEFAULT_PROMPT_TEMPLATE = (
    ...
    "7. 「保险期间」输出为「起期 至 止期」（用「 至 」连接），并同时单独输出「保险起期」「保险止期」。\n"
    "8. 键名必须与字段清单完全一致，不得增减或改名；所有值输出为字符串（未识别到的填 null）。\n"
    "\n"
    "输出格式参考示例（示例值仅演示格式，必须以图片实际内容为准，严禁照抄示例值）：\n"
    "{\n"
    "  \"文档类型\": \"保单\",\n"
    "  \"保单号\": \"PDAA202644020000123456\",\n"
    "  \"投保确认码\": \"A12B34CD\",\n"
    "  \"保险公司\": \"中国人民财产保险股份有限公司深圳市分公司\",\n"
    "  \"保险公司简称\": \"人保\",\n"
    "  \"险种类型\": \"商业险\",\n"
    "  \"被保险人\": \"张三\",\n"
    "  \"投保人\": \"张三\",\n"
    "  \"车主\": \"张三\",\n"
    "  \"证件号码\": \"440301199001011234\",\n"
    "  \"被保险人身份证号码\": \"440301199001011234\",\n"
    "  \"投保人身份证号码\": \"440301199001011234\",\n"
    "  \"车牌号\": \"粤B12345\",\n"
    "  \"车架号VIN\": \"LSVAA2183C2123456\",\n"
    "  \"发动机号\": \"A123456789\",\n"
    "  \"厂牌型号\": \"比亚迪牌QCJ7152BEV\",\n"
    "  \"核定载客\": \"5人\",\n"
    "  \"核定载质量\": null,\n"
    ...
    "  \"增值税额\": \"71.14\",\n"
    "  \"车船税\": \"360.00\",\n"
    "  \"保险期间\": \"2026年07月22日0时 至 2027年07月21日24时\",\n"
    "  \"保险起期\": \"2026年07月22日0时\",\n"
    "  \"保险止期\": \"2027年07月21日24时\",\n"
    "  \"签单日期\": \"2026年07月20日\",\n"
    "  \"收费确认时间\": \"2026年07月20日 15时30分22秒\",\n"
    "  \"销售渠道\": \"个人代理\",\n"
    "  \"中介机构\": \"深圳市某某保险代理有限公司\",\n"
    "  \"争议解决方式\": \"诉讼\",\n"
    "  \"制单人\": \"李四\",\n"
...
def query_ai_check_issues(db, filters: dict = None, page: int = 1, page_size: int = 20) -> dict:
    ...
    try:
        ...
        rows = [dict(r) for r in cursor.fetchall()]
        # ai_checked_at 是 DATETIME 对象，若不转成字符串直接 jsonify 会被 Flask
        # 默认 JSON encoder 序列化成 RFC1123/GMT 格式（如 "Tue, 21 Jul 2026 16:01:16 GMT"），
        # 该 "GMT" 标签具有误导性——数值其实就是数据库里的原始北京时间，并未做时区换算，
        # 前端会误当成真 UTC 再转本地时区导致显示偏差，这里统一转成字符串规避
        for r in rows:
