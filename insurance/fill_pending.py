...
# 注意：补充信息已入池暂存，保单识别成功后会自动补录，文案不能引导客户重发
MISS_TEMPLATE = (
    "⚠️ 补充信息未录入\n"
    "车牌：{plate}\n"
    "系统暂未找到该车牌的保单识别记录，您发送的补充信息（业务经理、业务专员、保险公司经办人等）已暂存。\n"
    "请检查：\n"
    "1️⃣ 该车牌的保单是否已发到本群——若未发，补发保单即可，识别成功后系统会自动录入暂存的补充信息，无需重发\n"
    "2️⃣ 车牌号是否输入有误——若有误，请按正确车牌重新发送一次补充信息\n"
    "若保单已发且车牌无误，请联系客服处理（已@客服跟进）。"
)
...
def _norm_plate(plate: str) -> str:
    """
    车牌归一化，复用 insurance.db 统一规则（繁体省份字转简体、去分隔符、大写）。

    必须与保单识别侧同规则：客户输入繁体"粵F2K375"而 PDF 解析出简体"粤F2K375"时，
    若入池车牌不转简体，match_and_apply 按车牌等值查询将永远匹配不上、无法自动补录。
    """
    from insurance.db import normalize_plate_keyword
    return normalize_plate_keyword(plate)
