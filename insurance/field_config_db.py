...
DEFAULT_COLUMNS = [
    ...
    {"key": "交强与交商终保时间", "visible": False, "order": 40, "display_name": "交强与交商终保时间"},
]
...
# 字段名本身已经区分了险种的差异字段（跟 MERGE_SPLIT_FIELDS 不同，这类字段不需要再生成
# "XX险+字段名"的新列，只应该从对应险种的那条子记录里取值）：比例规则(is_percent自定义列)
# 常见的"交强比例/商业比例/驾意比例"、"个人交强应得/个人商业应得/个人驾意应得"就是这种。
# 之前没有这张表时，这些字段落进"其他自定义字段互相补充"的通用兜底逻辑，兜底只认"组里第
# 一条有值的记录"，跟险种完全无关——同车牌合并了交强险+商业险+驾意险三条子记录时，"交强
# 比例"很容易被商业险/驾意险那条子记录的旧值/默认值抢先填掉，合并行显示的比例就跟单条
# 记录对不上（2026-07-21实测：单条交强险记录正确显示22%，合并行却还是默认的20%）
MERGE_TYPE_SPECIFIC_FIELDS = {
    "交强比例": ("compulsory",),
    "商业比例": ("commercial",),
    "驾意比例": ("accident", "non_vehicle"),
    "个人交强应得": ("compulsory",),
    "个人商业应得": ("commercial",),
    "个人驾意应得": ("accident", "non_vehicle"),
}
