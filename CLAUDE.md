...
- **新功能必须同步到 `run_ocr.py`**：`run_ocr.py` 是本地开发入口（仅保单识别，不含微信 SDK）。在 `main.py` 中新增的初始化（如建表、`init_xxx` 依赖注入）和页面路由，必须同步添加到 `run_ocr.py`，否则本地开发时新功能不可用（表未建、模块级 `_db` 为 None 静默失效、页面 404）
