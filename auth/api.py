...
def get_me():
    ...
    if user.get("role") != ROLE_SUPER_ADMIN and user.get("parent_id"):
        try:
            ...
            # 企业级「续保管理」开关（控制入口显示；账号级 users.renewal_enabled 仍控制是否收提醒）
            user["ent_renewal_enabled"] = int((_ent or {}).get("renewal_enabled") or 0)
        except Exception:
            user["seal_tracking_enabled"] = 0
            user["ent_renewal_enabled"] = 0
...
def api_list_enterprises():
    ...
    renewal_push_map = get_renewal_push_rooms_batch(_db)
    # 批量查推荐人名称
    ...
    for ent in enterprises:
        ...
        _push_rooms = renewal_push_map.get(ent["id"], [])
        ent["renewal_push_count"] = len(_push_rooms)
        ent["renewal_push_rooms"] = _push_rooms
